...
export async function GET(request: Request) {
  try {
    ...
    // Buscar todas as respostas que têm pelo menos um dado de contato
    ...
    // Agrupar respostas por respondente (usando email como chave principal, depois telefone, depois nome)
    const respondentsMap = new Map<string, any>();

    responses.forEach((response) => {
      // Criar uma chave única para o respondente
      const key =
      ...
      respondent.responses.push({
        ...
        answers: response.answers.map((answer) => {
          ...
          const question = response.campaign.questions.find((q: any) => q.id === answer.questionId);
