...
import { User, Mail, Phone, Calendar, MessageSquare, FileText, EyeIcon } from 'lucide-react';
...
export function RespondentsContent() {
  ...
  const fetchRespondents = async () => {
    try {
      ...
      const url = viewAsUserId
        ? `/api/respondents?viewAsUser=${viewAsUserId}`
        : '/api/respondents';
      const response = await fetch(url);
      if (!response.ok) throw new Error('Erro ao carregar respondentes');
      const data = await response.json();
      setRespondents(data);
      setFilteredRespondents(data);

      // Buscar nome do usuário se estiver visualizando como outro
      if (viewAsUserId) {
          ...
          <CardContent className="py-3">
            <div className="flex items-center gap-2 text-blue-700 dark:text-blue-300">
              ...
              <span className="font-medium">
                Visualizando respondentes de {viewingUserName}
              </span>
            </div>
          </CardContent>
        </Card>
      ...
      <div className="flex flex-col gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            Cadastros e histórico de participação nas pesquisas
          </p>
        </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-2xl font-bold">
                    {respondents.reduce((acc, r) => acc + r.totalResponses, 0)}
                  </p>
